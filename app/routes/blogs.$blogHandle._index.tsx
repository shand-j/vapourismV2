...
export const meta: MetaFunction<typeof loader> = ({data, location, params}) => {
  const blogHandle = params.blogHandle || 'news';
  ...
  if (data?.pageInfo) {
    ...
    if (data.pageInfo.hasNextPage && data.pageInfo.endCursor) {
      const nextUrl = `${baseUrl}/blogs/${blogHandle}?after=${encodeURIComponent(data.pageInfo.endCursor)}`;
      metaTags.push({tagName: 'link', rel: 'next', href: nextUrl});
    }
    
    // Add rel="prev" if we're not on the first page
    if (data.pageInfo.hasPreviousPage) {
      const prevUrl = `${baseUrl}/blogs/${blogHandle}`;
...
export default function BlogIndex() {
  const {articles, blogHandle, pageInfo} = useLoaderData<typeof loader>();
      ...
      <div className="container mx-auto px-4 py-16 lg:py-24">
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3 max-w-7xl mx-auto">
          {articles.map((article) => (
            <ArticleCard key={article.id} article={article} blogHandle={blogHandle} />
          ))}
        </div>
        ...
        {(pageInfo.hasNextPage || pageInfo.hasPreviousPage) && (
          <div className="mt-16 lg:mt-20 flex justify-center gap-4">
            ...
            {pageInfo.hasNextPage && pageInfo.endCursor && (
              <Link
                to={`/blogs/${blogHandle}?after=${encodeURIComponent(pageInfo.endCursor)}`}
                className="px-8 py-4 bg-gradient-to-r from-[#5b2be0] to-[#1fb2ff] text-white rounded-full hover:shadow-lg hover:shadow-[#5b2be0]/25 transition-all duration-300 font-semibold"
              >
                Next Page →
              </Link>
...
function ArticleCard({article, blogHandle}: {article: ShopifyArticle; blogHandle: string}) {
  ...
  return (
    <article className="group bg-white rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-slate-100 hover:border-slate-200">
      <Link to={`/blogs/${blogHandle}/${article.handle}`} className="block">
        {article.image?.url && (

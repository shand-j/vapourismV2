...
export const meta: MetaFunction<typeof loader> = ({data}) => {
  ...
  const {article, category, metaDescription, blogHandle} = data;
  ...
  const metaTags = [
    ...
    {property: 'og:url', content: `https://www.vapourism.co.uk/blogs/${blogHandle}/${article.handle}`},
...
export async function loader({params, request, context}: LoaderFunctionArgs) {
  ...
  const article = await getArticle(context.storefront, articleHandle, blogHandle);
  ...
  const structuredData = {
    ...
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': `${origin}/blogs/${blogHandle}/${article.handle}`,
    },
...
export default function BlogArticle() {
  const {article, blogHandle, category, structuredData} = useLoaderData<typeof loader>();
        ...
        <nav className="border-b border-slate-100 bg-white/80 backdrop-blur-sm sticky top-[var(--header-height,72px)] z-40">
          <div className="container mx-auto px-4 py-4">
            <ol className="flex items-center space-x-2 text-sm text-slate-500 max-w-4xl mx-auto">
              ...
              <li>
                <Link to={`/blogs/${blogHandle}`} className="hover:text-[#5b2be0] transition-colors">
                  Blog
                </Link>
              </li>
              <li className="text-slate-300">/</li>
